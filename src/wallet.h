...
enum AvailableCoinsType {
    ...
    ONLY_DENOMINATED = 2,
    ONLY_NOT10000IFMN = 3,
    ONLY_NONDENOMINATED_NOT10000IFMN = 4, // ONLY_NONDENOMINATED and not 10000 FDN at the same time
    ONLY_10000 = 5,                        // find masternode outputs including locked ones (use with caution)
    STAKABLE_COINS = 6                          // UTXO's that are valid for staking
};

// Possible states for zFDN send
enum ZerocoinSpendStatus {
    ZFDN_SPEND_OKAY = 0,                            // No error
    ZFDN_SPEND_ERROR = 1,                           // Unspecified class of errors, more details are (hopefully) in the returning text
    ZFDN_WALLET_LOCKED = 2,                         // Wallet was locked
    ZFDN_COMMIT_FAILED = 3,                         // Commit failed, reset status
    ZFDN_ERASE_SPENDS_FAILED = 4,                   // Erasing spends during reset failed
    ZFDN_ERASE_NEW_MINTS_FAILED = 5,                // Erasing new mints during reset failed
    ZFDN_TRX_FUNDS_PROBLEMS = 6,                    // Everything related to available funds
    ZFDN_TRX_CREATE = 7,                            // Everything related to create the transaction
    ZFDN_TRX_CHANGE = 8,                            // Everything related to transaction change
    ZFDN_TXMINT_GENERAL = 9,                        // General errors in MintToTxIn
    ZFDN_INVALID_COIN = 10,                         // Selected mint coin is not valid
    ZFDN_FAILED_ACCUMULATOR_INITIALIZATION = 11,    // Failed to initialize witness
    ZFDN_INVALID_WITNESS = 12,                      // Spend coin transaction did not verify
    ZFDN_BAD_SERIALIZATION = 13,                    // Transaction verification failed
    ZFDN_SPENT_USED_ZFDN = 14,                      // Coin has already been spend
    ZFDN_TX_TOO_LARGE = 15,                         // The transaction is larger than the max tx size
    ZFDN_SPEND_V1_SEC_LEVEL
};
...
public:
    ...
    void ZFdnBackupWallet();
    bool GetZerocoinKey(const CBigNum& bnSerial, CKey& key);
    bool CreateZFDNOutput(libzerocoin::CoinDenomination denomination, CTxOut& outMint, CDeterministicMint& dMint);
    bool GetMint(const uint256& hashSerial, CZerocoinMint& mint);
    bool GetMintFromStakeHash(const uint256& hashStake, CZerocoinMint& mint);
    bool DatabaseMint(CDeterministicMint& dMint);
    bool SetMintUnspent(const CBigNum& bnSerial);
    bool UpdateMint(const CBigNum& bnValue, const int& nHeight, const uint256& txid, const libzerocoin::CoinDenomination& denom);
    string GetUniqueWalletBackupName(bool fzfdnAuto) const;
    ...
    */
    ...
    CzFDNWallet* zwalletMain;

    bool fFileBacked;
    bool fWalletUnlockAnonymizeOnly;
    std::string strWalletFile;
    bool fBackupMints;
    std::unique_ptr<CzFDNTracker> zfdnTracker;
    ...
    void setZWallet(CzFDNWallet* zwallet)
    {
        zwalletMain = zwallet;
        zfdnTracker = std::unique_ptr<CzFDNTracker>(new CzFDNTracker(strWalletFile));
    }

    CzFDNWallet* getZWallet() { return zwalletMain; }
    ...
    void setZFdnAutoBackups(bool fEnabled)
    {
