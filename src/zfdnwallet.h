...
#ifndef FUNDIN_ZFDNWALLET_H
#define FUNDIN_ZFDNWALLET_H
...
public:
    CzFDNWallet(std::string strWalletFile);
    ...
    void GenerateDeterministicZFDN(libzerocoin::CoinDenomination denom, libzerocoin::PrivateCoin& coin, CDeterministicMint& dMint, bool fGenerateOnly = false);
    ...
    void UpdateCount();
    void Lock();
    void SeedToZFDN(const uint512& seed, CBigNum& bnValue, CBigNum& bnSerial, CBigNum& bnRandomness, CKey& key);

private:
    uint512 GetZerocoinSeed(uint32_t n);
};

#endif //FUNDIN_ZFDNWALLET_H
