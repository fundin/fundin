...
public:
    ...
    bool WriteZFDNSeed(const uint256& hashSeed, const vector<unsigned char>& seed);
    bool ReadZFDNSeed(const uint256& hashSeed, vector<unsigned char>& seed);
    bool ReadZFDNSeed_deprecated(uint256& seed);
    bool EraseZFDNSeed();
    bool EraseZFDNSeed_deprecated();

    bool WriteZFDNCount(const uint32_t& nCount);
    bool ReadZFDNCount(uint32_t& nCount);
