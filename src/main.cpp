...
#if defined(NDEBUG)
#error "Fundin cannot be compiled without assertions."
#endif
...
unsigned int nStakeMinAge = 3 * 60; // 3 Minutes Staking Time
int64_t nReserveBalance = 0;

/** Fees smaller than this (in ufdn) are considered zero fee (for relaying and mining)
...
bool ContextualCheckZerocoinSpend(const CTransaction& tx, const CoinSpend& spend, CBlockIndex* pindex, const uint256& hashBlock) {
    //Check to see if the zFDN is properly signed
    if (pindex->nHeight > Params().Zerocoin_LastOldParams()) {
        if (!spend.HasValidSignature())
            return error("%s: V2 zFDN spend does not have a valid signature", __func__);

        libzerocoin::SpendType expectedType = libzerocoin::SpendType::SPEND;
        if (tx.IsCoinStake())
            expectedType = libzerocoin::SpendType::STAKE;
        if (spend.getSpendType() != expectedType) {
            return error("%s: trying to spend zFDN without the correct spend type. txid=%s", __func__,
                         tx.GetHash().GetHex());
        }
    }
    ...
    if (IsSerialInBlockchain(spend.getCoinSerialNumber(), nHeightTx))
        return error("%s : zFDN spend with serial %s is already in block %d\n", __func__,
    ...
    if (!spend.HasValidSerial(paramsToUse))
        return error("%s : zFDN spend with serial %s from tx %s is not in valid range\n", __func__,
...

    {
        ...
        if(tx.IsZerocoinSpend()){
            ...
            if (IsTransactionInChain(tx.GetHash(), nHeightTx))
                return state.Invalid(error("AcceptToMemoryPool : zFDN spend tx %s already in block %d",
            ...
            for (const CTxIn& txIn : tx.vin) {
                ...
                if (!ContextualCheckZerocoinSpend(tx, spend, chainActive.Tip(), 0))
                    return state.Invalid(error("%s: ContextualCheckZerocoinSpend failed for tx %s", __func__,
                                               tx.GetHash().GetHex()), REJECT_INVALID, "bad-txns-invalid-zfdn");
            }
        } else {
            LOCK(pool.cs);
            ...
            // Check that zFDN mints are not already known
            if (tx.IsZerocoinMint()) {
                for (auto& out : tx.vout) {
                    if (!out.IsZerocoinMint())
                        continue;
...
{
    if (nHeight == 0) {
        return 41025000 * COIN;
    } else if (nHeight > 0 && nHeight <= 301) {
        return 1 * COIN;
    } else if (nHeight > 301 && nHeight <= 100101) {
        return 250 * COIN;
    } else if (nHeight > 100101 && nHeight <= 300101) {
        return 200 * COIN;
    } else if (nHeight > 300101 && nHeight <= 600101) {
        return 150 * COIN;
    } else if (nHeight > 600101 && nHeight <= 1200101) {
        return 100 * COIN;
    } else if (nHeight > 1200101 && nHeight <= 2200101) {
        return 75 * COIN;
    } else if (nHeight > 2200101 && nHeight <= 3200101) {
        return 50 * COIN;
    } else {
        return 2.5 * COIN;
    }
...
bool DisconnectBlock(CBlock& block, CValidationState& state, CBlockIndex* pindex, CCoinsViewCache& view, bool* pfClean)
{
    ...
    for (int i = block.vtx.size() - 1; i >= 0; i--) {
         ...
         * note we only undo zerocoin databasing in the following statement, value to and from Fundin
...
{
    RenameThread("fundin-scriptch");
    scriptcheckqueue.Thread();
}

void RecalculateZFDNMinted()
{
...
void RecalculateZFDNSpent()
{
    ...
    while (true) {
        ...
        //Remove spends from zFDN supply
...
bool RecalculateFDNSupply(int nHeightStart)
{
...
bool UpdateZFDNSupply(const CBlock& block, CBlockIndex* pindex)
{
    std::list<CZerocoinMint> listMints;
    BlockToZerocoinMintList(block, listMints);
...
//    LogPrintf("XX69----------> ConnectBlock(): nValueOut: %s, nValueIn: %s, nFees: %s, nMint: %s zFDNSpent: %s\n",
...
void static UpdateTip(CBlockIndex* pindexNew)
{
    ...
    // If turned on AutoZeromint will automatically convert FDN to zFDN
...
bool ContextualCheckBlock(const CBlock& block, CValidationState& state, CBlockIndex* const pindexPrev)
{
    ...
    if (nHeight > Params().Zerocoin_StartHeight()) {
        ...
        for (const CTransaction& tx : block.vtx) {
            ...
            // double check that there are no double spent zFDN spends in this block
            if (tx.IsZerocoinSpend()) {
                for (const CTxIn txIn : tx.vin) {
                    if (txIn.scriptSig.IsZerocoinSpend()) {
                        libzerocoin::CoinSpend spend = TxInToZerocoinSpend(txIn);
                        if (count(vBlockSerials.begin(), vBlockSerials.end(), spend.getCoinSerialNumber()))
                            return state.DoS(100, error("%s : Double spending of zFDN serial %s in block\n Block: %s",
...
bool ProcessNewBlock(CValidationState& state, CNode* pfrom, CBlock* pblock, CDiskBlockPos* dbp)
{
    ...
    if (nMints || nSpends)
        LogPrintf("%s : block contains %d zFDN mints and %d zFDN spends\n", __func__, nMints, nSpends);
...
bool static ProcessMessage(CNode* pfrom, string strCommand, CDataStream& vRecv, int64_t nTimeReceived)
{
    ...
    if (strCommand == NetMsgType::VERSION) {
        ...
        if (pfrom->cleanSubVer == "/Fundin Core:1.1.0/" ||
            pfrom->cleanSubVer == "/Fundin Core:1.3.0/" ||
            pfrom->cleanSubVer == "/Fundin Core:1.3.1/") {
            LOCK(cs_main);
        ...
        // Fundin: We use certain sporks during IBD, so check to see if they are
