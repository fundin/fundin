...
CzFDNWallet::CzFDNWallet(std::string strWalletFile)
{
    ...
    //Check for old db version of storing zfdn seed
    if (fFirstRun) {
        uint256 seed;
        if (walletdb.ReadZFDNSeed_deprecated(seed)) {
            //Update to new format, erase old
            seedMaster = seed;
            hashSeed = Hash(seed.begin(), seed.end());
            if (pwalletMain->AddDeterministicSeed(seed)) {
                if (walletdb.EraseZFDNSeed_deprecated()) {
                    LogPrintf("%s: Updated zFDN seed databasing\n", __func__);
                    fFirstRun = false;
                } else {
                    LogPrintf("%s: failed to remove old zfdn seed\n", __func__);
                }
            }
        }
    }
    ...
    if (fFirstRun) {
        ...
        seedMaster = seed;
        LogPrintf("%s: first run of zfdn wallet detected, new seed generated. Seedhash=%s\n", __func__, Hash(seed.begin(), seed.end()).GetHex());
...
bool CzFDNWallet::SetMasterSeed(const uint256& seedMaster, bool fResetCount)
{
    ...
    if (fResetCount)
        walletdb.WriteZFDNCount(nCountLastUsed);
    else if (!walletdb.ReadZFDNCount(nCountLastUsed))
        nCountLastUsed = 0;
...
void CzFDNWallet::Lock()
{
    seedMaster = 0;
}

void CzFDNWallet::AddToMintPool(const std::pair<uint256, uint32_t>& pMint, bool fVerbose)
{
    mintPool.Add(pMint, fVerbose);
}

//Add the next 20 mints to the mint pool
void CzFDNWallet::GenerateMintPool(uint32_t nCountStart, uint32_t nCountEnd)
{
    ...
    for (uint32_t i = n; i < nStop; ++i) {
        ...
        CKey key;
        SeedToZFDN(seedZerocoin, bnValue, bnSerial, bnRandomness, key);
...
bool CzFDNWallet::LoadMintPoolFromDB()
{
...
void CzFDNWallet::RemoveMintsFromPool(const std::vector<uint256>& vPubcoinHashes)
{
    for (const uint256& hash : vPubcoinHashes)
        mintPool.Remove(hash);
}

void CzFDNWallet::GetState(int& nCount, int& nLastGenerated)
{
...
void CzFDNWallet::SyncWithChain(bool fGenerateMintPool)
{
    ...
    while (found) {
        ...
        for (pair<uint256, uint32_t> pMint : listMints) {
            ...
            if (pwalletMain->zfdnTracker->HasPubcoinHash(pMint.first)) {
...
bool CzFDNWallet::SetMintSeen(const CBigNum& bnValue, const int& nHeight, const uint256& txid, const CoinDenomination& denom)
{
    ...
    CBigNum bnRandomness;
    CKey key;
    SeedToZFDN(seedZerocoin, bnValueGen, bnSerial, bnRandomness, key);
    ...
    // Add to zfdnTracker which also adds to database
    pwalletMain->zfdnTracker->Add(dMint, true);
...
void CzFDNWallet::SeedToZFDN(const uint512& seedZerocoin, CBigNum& bnValue, CBigNum& bnSerial, CBigNum& bnRandomness, CKey& key)
{
...
uint512 CzFDNWallet::GetZerocoinSeed(uint32_t n)
{
...
void CzFDNWallet::GenerateDeterministicZFDN(CoinDenomination denom, PrivateCoin& coin, CDeterministicMint& dMint, bool fGenerateOnly)
{
...
void CzFDNWallet::GenerateMint(const uint32_t& nCount, const CoinDenomination denom, PrivateCoin& coin, CDeterministicMint& dMint)
{
...
bool CzFDNWallet::RegenerateMint(const CDeterministicMint& dMint, CZerocoinMint& mint)
{
