...
WalletModel::SendCoinsReturn WalletModel::prepareTransaction(WalletModelTransaction& transaction, const CCoinControl* coinControl)
{
    ...
    foreach (const SendCoinsRecipient& rcp, recipients) {
        if (rcp.paymentRequest.IsInitialized()) { // PaymentRequest...
            ...
            total += subtotal;
        } else { // User-entered fundin address / amount:
...

    {
        ...
        if (recipients[0].useSwiftTX && total > GetSporkValue(SPORK_5_MAX_VALUE) * COIN) {
            emit message(tr("Send Coins"), tr("SwiftX doesn't support sending values that high yet. Transactions are currently limited to %1 FDN.").arg(GetSporkValue(SPORK_5_MAX_VALUE)),
...
WalletModel::SendCoinsReturn WalletModel::sendCoins(WalletModelTransaction& transaction)
{
...

    {
        ...
        foreach (const SendCoinsRecipient& rcp, recipients) {
            ...
            } else if (!rcp.message.isEmpty()) // Message from normal fundin:URI (fundin:XyZ...?message=example)
            {
...
bool WalletModel::setWalletEncrypted(bool encrypted, const SecureString& passfdnase)
{
    if (encrypted) {
        // Encrypt
        return wallet->EncryptWallet(passfdnase);
    } else {
...
bool WalletModel::setWalletLocked(bool locked, const SecureString& passFdnase, bool anonymizeOnly)
{
    if (locked) {
        // Lock
        ...
        // Unlock
        return wallet->Unlock(passFdnase, anonymizeOnly);
    }
}
...
bool WalletModel::changePassfdnase(const SecureString& oldPass, const SecureString& newPass)
{
    bool retval;
    {
        LOCK(wallet->cs_wallet);
        wallet->Lock(); // Make sure wallet is locked before attempting pass change
        retval = wallet->ChangeWalletPassfdnase(oldPass, newPass);
    }
    return retval;
}
...
{
    setMints.clear();
    setMints = pwalletMain->zfdnTracker->ListMints(fUnusedOnly, fMaturedOnly, fUpdateStatus);
}
