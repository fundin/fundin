                                          ...
                                          banTableContextMenu(0)
{
    ...
    std::string strzfdnPathCustom = GetArg("-zfdnbackuppath", "");
    ...
    if(!strzfdnPathCustom.empty()) {
        ui->wallet_customzfdnbackuppath->setText(QString::fromStdString(strzfdnPathCustom));
        ui->wallet_customzfdnbackuppath_label->setVisible(true);
        ui->wallet_customzfdnbackuppath->setVisible(true);
    }

    if((!strPathCustom.empty() || !strzfdnPathCustom.empty()) && nCustomBackupThreshold > 0) {
    ...
    startExecutor();
    setTrafficGrafdNange(INITIAL_TRAFFIC_GRAPH_MINS);
...
void RPCConsole::clear()
{
    ...
    message(CMD_REPLY, (tr("Welcome to the Fundin RPC console.") + "<br>" +
...
void RPCConsole::on_sldGrafdNange_valueChanged(int value)
{
    const int multiplier = 5; // each position on the slider represents 5 min
    int mins = value * multiplier;
    setTrafficGrafdNange(mins);
}
...
void RPCConsole::setTrafficGrafdNange(int mins)
{
    ui->trafficGraph->setGrafdNangeMins(mins);
    ui->lblGrafdNange->setText(GUIUtil::formatDurationStr(mins * 60));
}
