...
#include "ui_zfdncontroldialog.h"
...
std::set<std::string> ZFdnControlDialog::setSelectedMints;
std::set<CMintMeta> ZFdnControlDialog::setMints;

ZFdnControlDialog::ZFdnControlDialog(QWidget *parent) :
    QDialog(parent),
    ui(new Ui::ZFdnControlDialog),
    model(0)
{
    ui->setupUi(this);
    setMints.clear();
...
ZFdnControlDialog::~ZFdnControlDialog()
{
    delete ui;
}

void ZFdnControlDialog::setModel(WalletModel *model)
{
    this->model = model;
    updateList();
}
...
void ZFdnControlDialog::updateList()
{
...
void ZFdnControlDialog::updateSelection(QTreeWidgetItem* item, int column)
{
...
void ZFdnControlDialog::updateLabels()
{
    int64_t nAmount = 0;
    for (const CMintMeta& mint : setMints) {
    ...
    ui->labelZFdn_int->setText(QString::number(nAmount));
    ui->labelQuantity_int->setText(QString::number(setSelectedMints.size()));

    //update PrivacyDialog labels
    privacyDialog->setZFdnControlLabels(nAmount, setSelectedMints.size());
}

std::vector<CMintMeta> ZFdnControlDialog::GetSelectedMints()
{
...
void ZFdnControlDialog::ButtonAllClicked()
{
    ui->treeWidget->blockSignals(true);
