...
void OptionsModel::Init()
{
    ...
    if (!settings.contains("nDisplayUnit"))
        settings.setValue("nDisplayUnit", BitcoinUnits::FDN);
    ...
    if (!settings.contains("nAnonymizeFundinAmount"))
        settings.setValue("nAnonymizeFundinAmount", 1000);

    nAnonymizeFundinAmount = settings.value("nAnonymizeFundinAmount").toLongLong();
...
// Wallet
    ...
    if (settings.contains("nAnonymizeFundinAmount"))
        SoftSetArg("-anonymizefundinamount", settings.value("nAnonymizeFundinAmount").toString().toStdString());
...
void OptionsModel::Reset()
{
    ...
    settings.clear();
    resetSettings = true; // Needed in fundin.cpp during shotdown to also remove the window positions
...
QVariant OptionsModel::data(const QModelIndex& index, int role) const
{
    if (role == Qt::EditRole) {
        ...
        case AnonymizeFundinAmount:
            return QVariant(nAnonymizeFundinAmount);
        case Listen:
...
bool OptionsModel::setData(const QModelIndex& index, const QVariant& value, int role)
{
    ...
    if (role == Qt::EditRole) {
        ...
        case AnonymizeFundinAmount:
            nAnonymizeFundinAmount = value.toInt();
            settings.setValue("nAnonymizeFundinAmount", nAnonymizeFundinAmount);
            emit anonymizeFundinAmountChanged(nAnonymizeFundinAmount);
            break;
        case CoinControlFeatures:
