...
// Copyright (c) 2017-2018 The Fundin developers
...
class TxViewDelegate : public QAbstractItemDelegate
{
    Q_OBJECT
public:
    TxViewDelegate() : QAbstractItemDelegate(), unit(BitcoinUnits::FDN)
    {
    }
...
void OverviewPage::getPercentage(CAmount nUnlockedBalance, CAmount nZerocoinBalance, QString& sFDNPercentage, QString& szFDNPercentage)
{
    int nPrecision = 2;
    ...
    szFDNPercentage = "(" + QLocale(QLocale::system()).toString(dzPercentage, 'f', nPrecision) + " %)";
    sFDNPercentage = "(" + QLocale(QLocale::system()).toString(dPercentage, 'f', nPrecision) + " %)";
                              ...
                              const CAmount& watchOnlyBalance, const CAmount& watchUnconfBalance, const CAmount& watchImmatureBalance)
{
    ...
    CAmount fdnAvailableBalance = balance - immatureBalance - nLockedBalance;
    ...
    // Combined balances
    CAmount availableTotalBalance = fdnAvailableBalance + matureZerocoinBalance;
    CAmount sumTotalBalance = nTotalBalance + zerocoinBalance;

    // FDN labels
    ui->labelBalance->setText(BitcoinUnits::floorHtmlWithUnitComma(nDisplayUnit, fdnAvailableBalance, false, BitcoinUnits::separatorAlways));
    ...
    // Percentage labels
    ui->labelFDNPercent->setText(sPercentage);
    ui->labelzFDNPercent->setText(szPercentage);

    // Adjust bubble-help according to AutoMint settings
    QString automintHelp = tr("Current percentage of zFDN.\nIf AutoMint is enabled this percentage will settle around the configured AutoMint percentage (default = 10%).\n");
    bool fEnableZeromint = GetBoolArg("-enablezeromint", false);
    int nZeromintPercentage = GetArg("-zeromintpercentage", 10);
    if (fEnableZeromint) {
        automintHelp += tr("AutoMint is currently enabled and set to ") + QString::number(nZeromintPercentage) + "%.\n";
        automintHelp += tr("To disable AutoMint add 'enablezeromint=0' in fundin.conf.");
    }
    else {
        automintHelp += tr("AutoMint is currently disabled.\nTo enable AutoMint change 'enablezeromint=0' to 'enablezeromint=1' in fundin.conf");
    }
    ...
    bool showFDNAvailable = settingShowAllBalances || fdnAvailableBalance != nTotalBalance;
    bool showWatchOnlyFDNAvailable = watchOnlyBalance != nTotalWatchBalance;
    bool showFDNPending = settingShowAllBalances || unconfirmedBalance != 0;
    bool showWatchOnlyFDNPending = watchUnconfBalance != 0;
    bool showFDNLocked = settingShowAllBalances || nLockedBalance != 0;
    bool showWatchOnlyFDNLocked = nWatchOnlyLockedBalance != 0;
    bool showImmature = settingShowAllBalances || immatureBalance != 0;
    bool showWatchOnlyImmature = watchImmatureBalance != 0;
    bool showWatchOnly = nTotalWatchBalance != 0;
    ui->labelBalance->setVisible(showFDNAvailable || showWatchOnlyFDNAvailable);
    ui->labelBalanceText->setVisible(showFDNAvailable || showWatchOnlyFDNAvailable);
    ui->labelWatchAvailable->setVisible(showFDNAvailable && showWatchOnly);
    ui->labelUnconfirmed->setVisible(showFDNPending || showWatchOnlyFDNPending);
    ui->labelPendingText->setVisible(showFDNPending || showWatchOnlyFDNPending);
    ui->labelWatchPending->setVisible(showFDNPending && showWatchOnly);
    ui->labelLockedBalance->setVisible(showFDNLocked || showWatchOnlyFDNLocked);
    ui->labelLockedBalanceText->setVisible(showFDNLocked || showWatchOnlyFDNLocked);
    ui->labelWatchLocked->setVisible(showFDNLocked && showWatchOnly);
    ...
    bool showzFDNAvailable = settingShowAllBalances || zerocoinBalance != matureZerocoinBalance;
    bool showzFDNUnconfirmed = settingShowAllBalances || unconfirmedZerocoinBalance != 0;
    bool showzFDNImmature = settingShowAllBalances || immatureZerocoinBalance != 0;
    ui->labelzBalanceMature->setVisible(showzFDNAvailable);
    ui->labelzBalanceMatureText->setVisible(showzFDNAvailable);
    ui->labelzBalanceUnconfirmed->setVisible(showzFDNUnconfirmed);
    ui->labelzBalanceUnconfirmedText->setVisible(showzFDNUnconfirmed);
    ui->labelzBalanceImmature->setVisible(showzFDNImmature);
    ui->labelzBalanceImmatureText->setVisible(showzFDNImmature);
    bool showPercentages = ! (zerocoinBalance == 0 && nTotalBalance == 0);
    ui->labelFDNPercent->setVisible(showPercentages);
    ui->labelzFDNPercent->setVisible(showPercentages);
...
void OverviewPage::setWalletModel(WalletModel* model)
{
    ...
    // update the display unit, to not use the default ("FDN")
    updateDisplayUnit();
}
