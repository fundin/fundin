...
VERSION=
commit=false
url=https://github.com/fundin/fundin
proc=2
mem=2000
lxc=true
...
Run this script from the directory containing the fundin, gitian-builder, gitian.sigs, and fundin-detached-sigs.
...
-u|--url	Specify the URL of the repository. Default is https://github.com/fundin/fundin
...
then
	# Make output folder
	mkdir -p "./fundin-binaries/${VERSION}"
	...
	then
	    ...
	    echo ""
	    ./bin/gbuild -j ${proc} -m ${mem} --commit fundin=${COMMIT} --url fundin=${url} ../fundin/contrib/gitian-descriptors/gitian-linux.yml
	    ./bin/gsign --signer $SIGNER --release ${VERSION}-linux --destination ../gitian.sigs/ ../fundin/contrib/gitian-descriptors/gitian-linux.yml
	    mv build/out/fundin-*.tar.gz build/out/src/fundin-*.tar.gz ../fundin-binaries/${VERSION}
	fi
	# Windows
	if [[ $windows = true ]]
	then
	    ...
	    echo ""
	    ./bin/gbuild -j ${proc} -m ${mem} --commit fundin=${COMMIT} --url fundin=${url} ../fundin/contrib/gitian-descriptors/gitian-win.yml
	    ...
	    mv build/out/fundin-*.zip build/out/fundin-*.exe ../fundin-binaries/${VERSION}
	fi
	# Mac OSX
	if [[ $osx = true ]]
	then
	    ...
	    echo ""
	    ./bin/gbuild -j ${proc} -m ${mem} --commit fundin=${COMMIT} --url fundin=${url} ../fundin/contrib/gitian-descriptors/gitian-osx.yml
	    ...
	    mv build/out/fundin-*.tar.gz build/out/fundin-*.dmg ../fundin-binaries/${VERSION}
	fi
	# AArch64
	if [[ $aarch64 = true ]]
	then
	    ...
	    echo ""
	    ./bin/gbuild -j ${proc} -m ${mem} --commit fundin=${COMMIT} --url fundin=${url} ../fundin/contrib/gitian-descriptors/gitian-aarch64.yml
	    ./bin/gsign --signer $SIGNER --release ${VERSION}-aarch64 --destination ../gitian.sigs/ ../fundin/contrib/gitian-descriptors/gitian-aarch64.yml
	    mv build/out/fundin-*.tar.gz build/out/src/fundin-*.tar.gz ../fundin-binaries/${VERSION}
	fi
	popd || exit
...
then
	...
	then
	    ...
	    echo ""
	    ./bin/gbuild -i --commit signature=${COMMIT} ../fundin/contrib/gitian-descriptors/gitian-win-signer.yml
	    ...
	    mv build/out/fundin-*win64-setup.exe ../fundin-binaries/${VERSION}
	    mv build/out/fundin-*win32-setup.exe ../fundin-binaries/${VERSION}
	fi
	# Sign Mac OSX
	if [[ $osx = true ]]
	then
	    ...
	    echo ""
	    ./bin/gbuild -i --commit signature=${COMMIT} ../fundin/contrib/gitian-descriptors/gitian-osx-signer.yml
	    ...
	    mv build/out/fundin-osx-signed.dmg ../fundin-binaries/${VERSION}/fundin-${VERSION}-osx.dmg
	fi
	popd || exit
